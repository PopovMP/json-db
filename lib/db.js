...
import {writeAndForget, setErrorHandler, isWriterBusy} from "@popovmp/file-writer";
...
export class JsonDB {
  ...
  save() {
    ...
    const content  = this.stringifyDb();
    writeAndForget(filepath, content);
  }

  /**
   * Stringifies the DB content one-record-per-line
   * @returns { string }
   */
  stringifyDb() {
      /** @type { string[] } */
      ...
      for (const id of Object.keys(this.docMap)) {
        /** @type { Doc } */
        const doc = this.docMap[id];

        /** @type { string } */
        const rec = JSON.stringify(doc);
