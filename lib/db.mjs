...
import {writeAndForget}    from "@popovmp/file-writer";
...
/** @typedef {import("../index.mjs").Doc          } Doc           */
 ...
 * @param { Record<string, any> } opSet - {opKey: opVal, ...}
 *
 * @return { boolean }
 */
function evalOperatorSet(value, opSet) {
    for (const key of Object.keys(opSet)) {
        if (!evalOperator(value, key, opSet[key])) {
            return false;
        }
    }
    ...
    switch (opKey) {
        ...
        case "$like":
            if (typeof opVal === "string" && typeof value === "string") {
                return new RegExp(opVal, "i").test(value);
            }
            return false;
        case "$type": {
            if (opVal === "array") {
...
export function dbProjection(doc, projection) {
    ...
    const output = /** @type {import("../index.mjs").Doc} */ ({});
...
export function getDb(dbName) {
    ...
    if (!dbHolder[dbName]) {
        ...
        if (!existsSync(fileName)) {
            logError("Database not found", "json-db :: getDb");
        ...
        try {
            ...
            logError(err.message, "json-db :: getDb");
